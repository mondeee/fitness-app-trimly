...
import { dogBreedsSeedsQueryKey, fetchDogBreedsGalleryPage, } from '@/lib/api/dogBreedsGalleryApi';
...
export function useDogBreedsGalleryInfinite() {
    ...
    const query = useInfiniteQuery({
        ...
        initialPageParam: 0,
        queryFn: ({ pageParam, signal }) => fetchDogBreedsGalleryPage(queryClient, pageParam, PAGE_SIZE, signal),
        getNextPageParam: (lastPage) => lastPage.nextOffset < lastPage.total ? lastPage.nextOffset : undefined,
        staleTime: 1000 * 60 * 10,
    });
    const flatItems = useMemo(() => query.data?.pages.flatMap((p) => p.items) ?? [], [query.data?.pages]);
